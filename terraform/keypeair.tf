...
resource "aws_key_pair" "generated_key" {
  key_name   = "chave-fiap-ms-pedido"                         # Nome da chave que será utilizada para acessar a instância EC2
...
resource "aws_secretsmanager_secret" "pedido" {
  name = "keypair-ms-pedido-v1" # Nome do segredo que será armazenado
}

# Recurso para armazenar a versão do segredo no AWS Secrets Manager
resource "aws_secretsmanager_secret_version" "pedido" {
  secret_id = aws_secretsmanager_secret.pedido.id # ID do segredo que estamos atualizando
